...
function OnPlayerTryTakePilotVehicle( pPlayer )
	...
	if iTimeLeft >= 5 then
		...
		triggerEvent( "PlayerAction_Task_Pilot_1_step_1", pPlayer )
	else
		triggerEvent( "PlayerAction_Task_Pilot_1_step_1", pPlayer )
		triggerEvent( "PlayerAction_Task_Pilot_1_step_2", pPlayer )
	end
...
function onPilotMarkerPass_handler( player, is_return, distance_mul, is_perfect )
	if is_return then 
		distance_mul = 0.5
	else
		distance_mul = distance_mul / 2
	end
	...
	money, exp = math.floor( money * distance_mul * (is_perfect and 1.04 or 1) ), math.floor( exp * distance_mul )
