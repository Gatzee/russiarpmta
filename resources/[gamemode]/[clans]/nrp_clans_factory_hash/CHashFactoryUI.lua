...
function ShowFactoryUI( state, data )
    if state then
        ShowFactoryUI( false )
        ibInterfaceSound()
        showCursor( true )
        ...
        UI.black_bg = ibCreateBackground( 0xBF1D252E, ShowFactoryUI, true, true )
        ...
        UI.lbl_header = ibCreateLabel( 30, 0, 0, 0, "Сушка петрушки", UI.head_bg, COLOR_WHITE, 1, 1, "left", "center", ibFonts.bold_20 )
            :center_y( )
        ...
        ibCreateImage( 0, 29, 30, 30, "img/icon_hash.png", UI.ready_area )
        UI.ready_text_lbl = ibCreateLabel( 45, 27, 0, 0, "Готовая петрушка:", UI.ready_area, COLOR_WHITE, 1, 1, "left", "top", ibFonts.regular_14 )
        ...
        UI.btn_takeall = ibCreateButton( 45 - 10, 45, 108, 22, UI.ready_area, "img/btn_takeall.png", _, _, 0x8FFFFFFF, 0xFFFFFFFF, 0xFFAAAAAA )
            :ibOnClick( function( button, state )
                if button ~= "left" or state ~= "up" then return end
                if UI.ready_lbl:ibData( "text" ) == 0 then return end
                ibClick( )
                triggerServerEvent( "CHF:onPlayerWantTakeAllProducts", resourceRoot )
            end )
        ...
        ibCreateLabel( 0, 120, 0, 0, "Производственный «Цех-Петрушки»", UI.head_bg, COLOR_WHITE, 1, 1, "center", "center", ibFonts.bold_18 ):center_x( )
        ibCreateLabel( 0, 147, 0, 0, "Время, которое осталось до конца сушки сырья", UI.head_bg, COLOR_WHITE, 1, 1, "center", "center", ibFonts.regular_16 ):center_x( )
            ...
            elseif not item then
                ...
                local hover_img = ibCreateImage( 0, 29, 147, 125, _, slot_bg, 0xD81f2934 )
                    ...
                    :ibOnClick( function( button, state )
                        ...
                        if localPlayer:InventoryGetItemCount( IN_HASH_RAW ) <= 0 then
                            localPlayer:ShowError( "У тебя нет шишки-петрушки" )
                            return
                        end
                        triggerServerEvent( "CHF:onPlayerWantAddRawMaterial", resourceRoot, slot )
                    end )
            ...
            -- Готово
                ...
                ibCreateArea( 0, 29, 147, 125, slot_bg )
                    ...
                    :ibOnClick( function( button, state )
                        if button ~= "left" or state ~= "up" then return end
                        ibClick( )
                        triggerServerEvent( "CHF:onPlayerWantTakeProduct", resourceRoot, slot )
                    end )
            end
            ...
            :ibOnClick( function( button, state )
                ...
                for slot, item in pairs( DATA.items ) do
                    if not item.finish_ts then
                        triggerServerEvent( "CHF:onPlayerWantStartMakingProduct", resourceRoot )
                        return
                    end
                end

                localPlayer:ShowError( "Ты не поставил сырье для сушки" )
            end )
    else
...
addEvent( "CHF:ShowUI", true )
addEventHandler( "CHF:ShowUI", resourceRoot, ShowFactoryUI )
...
addEvent( "CHF:UpdateUI", true )
addEventHandler( "CHF:UpdateUI", resourceRoot, UpdateUI )