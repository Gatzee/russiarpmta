...
MEDIC_MURDER_DATA = {
	[1] = {
		position = Vector3( -927.20001, -1816.5, 20.8 );
		ped_position = Vector3( -926.5, -1801.59998, 21 );
	};
	[2] = {
		position = Vector3( -1187.2, -1725.20001, 21 );
		ped_position = Vector3( -1182.4, -1730.90002, 21 );
	};
	[3] = {
		position = Vector3( -909, -1507.09998, 21 );
		ped_position = Vector3( -887.5, -1513.29999, 21 );
	};
	[4] = {
		position = Vector3( 627.70001, -1914.8, 21 );
	...
	tasks = {
		[1] = {
			...
			Setup = {
				client = function( data )
					...
					}, "training_murder_nsk_medic_end_step_1", _, true )
				end;
		...
		[2] = {
			...
			Setup = {
				client = function( data )
					local number = data.random_number % CONST_COUNT_POINTS + 1
					CreateQuestPoint( MEDIC_MURDER_DATA[ number ].position, "training_murder_nsk_medic_end_step_2", _, _, 0, 0 )
				end;
			...
			Setup = {
				client = function( data )
					...
					CreateQuestPoint( MEDIC_MURDER_DATA[ number ].ped_position, function()
						...
						triggerServerEvent( "training_murder_nsk_medic_end_step_3", localPlayer )
					end, _, 1.5, 0, 0 )
				end;
		...
		[4] = {
			...
			Setup = {
				client = function( data )
					...
					CreateQuestPoint( vehicle.position, "training_murder_nsk_medic_end_step_4", _, _, 0, 0 )
				end;
			};
		};
		[5] = {
			...
			Setup = {
				client = function( data )
					...
					CreateQuestPoint( MEDIC_MURDER_DATA[ number ].ped_position, function()
						...
						triggerServerEvent( "training_murder_nsk_medic_end_step_5", localPlayer )
					end, _, 1.5, 0, 0 )
				end;
			};
		...
		[6] = {
			...
			Setup = {
				client = function( data )
					local vehicle = localPlayer:getData( "quest_vehicle" )
					CreateQuestPoint( vehicle.position, "training_murder_nsk_medic_end_step_6", _, _, 0, 0 )
				end;
			};
		...
		[7] = {
			...
			Setup = {
				client = function( data )
					CreateQuestPoint( Vector3( 432.161, -2476.533, 21.223 ), "training_murder_nsk_medic_end_step_7" )
				end;
			};
		};
	};
