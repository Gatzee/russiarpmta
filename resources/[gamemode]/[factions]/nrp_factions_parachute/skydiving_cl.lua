				...
				if not getElementData(localPlayer,"skydiving") then
					if not processLineOfSight ( x,y,z, x,y,z-MIN_GROUND_HEIGHT, true, true,false,true,true,false,false,false,localPlayer ) and not testLineAgainstWater ( x,y,z - MIN_GROUND_HEIGHT,x,y,z) then
						...
						if currentTick > (divingTick + t(900)) then
							setElementData( localPlayer,"skydiving",true, false )
...
function stopSkyDiving()
	...
	setElementData( localPlayer,"skydiving", false, false)
...
function warningText()
	warning = true
	dxDrawText ( "Открой свой парашют!", 
...
local function checkRenderHandler( is_skydiving )
	if ( is_skydiving ) then
		...
		if ( is_render_handled and not next( g_skydivers ) and getPedWeapon ( localPlayer ) ~= 46 ) then
...
function updateSkyDiving( data, oldva, is_skydiving )
	if ( data == "skydiving" ) then
		g_skydivers[source] = is_skydiving or nil

		checkRenderHandler( is_skydiving )
	end
end
...
local function checkSwitchedToParachute( )
	if ( getPedWeaponSlot ( localPlayer ) == 11 ) then
		if ( getPedWeapon ( localPlayer ) == 46 ) then
			checkRenderHandler( true )
...
function onClientPlayerWeaponSwitch_handler( prevSlot, newSlot )
	if ( newSlot == 11 ) then
		if ( getPedWeapon ( localPlayer ) == 46 ) then
			checkRenderHandler( true )
		else
			setTimer( checkSwitchedToParachute, 500, 0 )
		end
	elseif ( prevSlot == 11 ) then
		checkRenderHandler( )
	end
end
addEventHandler ( "onClientPlayerWeaponSwitch", localPlayer, onClientPlayerWeaponSwitch_handler )