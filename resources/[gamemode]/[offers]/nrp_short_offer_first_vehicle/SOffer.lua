...
ANALYTICS_OFFER_NAME = "first_vehicle_fast_offer"
...
function initOffer( player )
    ...
    triggerClientEvent( player, "onPlayerShortOfferFV", player )
...
addEventHandler( "onPlayerGotNewVehicle", root, function( vehicle )
    ...
    for i, veh in pairs( source:GetVehicles( false, false, true ) ) do
...
addEvent( "onPlayerWantBuyCasesFV", true )
addEventHandler( "onPlayerWantBuyCasesFV", resourceRoot, function ( )
    ...
    if client:TakeDonate( pack_price, "sale", ANALYTICS_OFFER_NAME ) then
        ...
        SendElasticGameEvent( client:GetClientID( ), "fast_offer_purchase", {
            ...
            reward = toJSON( {
                ...
                { item_name = "tuning_case", quantity = tuning_case_v, cost = tuning_case_v * data.class * 100 + 49, currency = "hard" },
            } ),
        } )
