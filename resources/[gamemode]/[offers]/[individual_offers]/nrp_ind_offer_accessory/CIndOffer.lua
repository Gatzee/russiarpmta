...
function ShowOffer( offer_data )
	...
	local l_soft_count = ibCreateLabel( 312, 312, 0, 0, format_price( OFFER.items[ 1 ].count ), UI.bg, _, _, _, "right", "center", ibFonts.oxaniumbold_22 )

	local item2 = OFFER.items[ 2 ]
	local item2_name = CONST_ACCESSORIES_INFO[ item2.id ].name
	local l_item2_name = ibCreateLabel( 734, 314, 0, 0, item2_name, UI.bg, _, _, _, "center", "center", ibFonts.bold_18 )
	local area_item = ibCreateArea( 739, 433, 0, 0, UI.bg )
	local img_item = ibCreateContentImage( 0, 0, 300, 180, item2.type, item2.id, area_item )
		:ibSetInBoundSize( 220 ):center( )
