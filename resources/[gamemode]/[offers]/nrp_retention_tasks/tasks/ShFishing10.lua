...
TASKS_CONFIG[ id ] = {
    id = id,
    name = "Рыбалка на NextRP",
    reward = 7000,

    desc = "Продай 10 кг рыбы",

    required_weight = 10,

    fns = {
        add_fishing_weight = function( hobby, weight )
            if hobby ~= 1 then return end
    ...
    fn_complete = {
        server = function( self, player, data )
            player:GiveMoney( self.reward, "retention_task", "fishing10" )
        end,
...
addEventHandler( "HB:OnPlayerSellItems", root, GetSelf( ).fns.add_fishing_weight )