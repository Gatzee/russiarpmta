...
TASKS_CONFIG[ id ] = {
    ...
    desc = "Отработай 4 часа таксистом в компании",
    ...
    fns = {
        ping_pulse = function( player )
            ...
            if not self.required_job[ player:GetJobClass( ) ] then return end
        ...
        stop_pulse = function( player )
            local self = GetSelf( )
            if ( self.timers[ player ] and isTimer( self.timers[ player ] ) ) then killTimer( self.timers[ player ] ) end
    ...
    fn_complete = {
        server = function( self, player, data )
            player:GiveMoney( self.reward, "retention_task", "taxi" )
        end,
