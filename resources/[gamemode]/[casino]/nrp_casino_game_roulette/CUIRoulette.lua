...
function ShowUI_Game( state, data )
	if state then
		...
		for i, v in pairs(temp_scores) do
			ui["player_bg"..i] = ibCreateImage( px, py, 180, 50, "img/bg_user.png", ui.scores_bg)
		...
		-- Подсказки
		ui.shoot_hint = ibCreateImage( scx/2-396/2, scy-300, 396, 34, "img/shoot_hint.png", false ):ibData("alpha", 0)

		-- Угловые подсказки
		ui.corner_hints = ibCreateImage( 10, scy-30, 94, 18, "img/corner_hints.png", false ):ibData("alpha", 0):ibAlphaTo(255, 1000)
...
function UpdateTurnPlayer( turn_player )
	for k,v in pairs(temp_scores) do
