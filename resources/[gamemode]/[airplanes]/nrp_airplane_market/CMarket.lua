...
function CreateMarket( config, market_id )
	...
	store.PostJoin = function(store) 
		...
		triggerServerEvent( "onPlayerAirplaneMarketOpen", localPlayer, tonumber( config.assortment_id ) )
	end
	...
	store.elements = {}
	store.elements.blip = Blip( config.x, config.y, config.z, 5, 2, 255, 0, 0, 255, 0, 300 )
end