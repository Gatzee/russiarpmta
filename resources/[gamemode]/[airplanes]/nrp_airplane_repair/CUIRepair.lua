...
function ShowUI_Workshop( state, data )
	...
	if state then
		...
		local iRefillCost = pData.vehicle:IsInFaction() and 0 or math.floor( iSpaceLeft * FUEL_COST )
		...
		local pWorkshop = WORKSHOPS_LIST[ data.workshop_id ]
		...
		for k, part in pairs(pData.details) do
			if k ~= "refill" then
				...
				local part_name = ibCreateLabel( 0, 0, 0, 30, pPartsConf[k], item, 0xFFFFFFFF, 1, 1, "left", "center" ):ibData("font", ibFonts.regular_18)
				local operation = ibCreateLabel( 200, 0, 0, 30, "Восстановление", item, 0xFFffde9e, 1, 1, "left", "center" ):ibData("font", ibFonts.regular_16)
				local cost = ibCreateLabel( 430, 0, 0, 30, format_price( part.cost ), item, 0x70FFFFFF, 1, 1, "left", "center" ):ibData("font", ibFonts.regular_16)
				local icon_money = ibCreateImage( 390, 4 , 27, 23, "files/img/icon_money.png", item )
		...
		ui.label_refill = ibCreateLabel( 80, 550, 0, 0, "ЗАПРАВКА АВИАТРАНСПОРТА:", ui.main, 0x70FFFFFF, 1, 1, "left", "center" ):ibData("font", ibFonts.bold_16)
		local item = ibCreateImage( 80, 590, 500, 30, nil, ui.main, 0x00000000 )
		local part_name = ibCreateLabel( 0, 0, 0, 30, "Заправить полный бак", item, 0xFFFFFFFF, 1, 1, "left", "center" ):ibData("font", ibFonts.regular_18)
		local cost = ibCreateLabel( 430, 0, 0, 30, format_price( pData.details.refill.cost ), item, 0x70FFFFFF, 1, 1, "left", "center" ):ibData("font", ibFonts.regular_16)
		local icon_money = ibCreateImage( 390, 4 , 27, 23, "files/img/icon_money.png", item )
		local btn_select = ibCreateButton( 550, 4, 26, 26, item, "files/img/btn_select.png", "files/img/btn_select_active.png", "files/img/btn_select_active.png", 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF  )
		:ibOnClick( function( button, state ) 
			if button == "left" and state == "down" then
		...
		:ibOnClick( function( button, state ) 
			if button == "left" and state == "down" then
				...
				triggerServerEvent( "OnPlayerTryRepairSpecialVehicle", localPlayer, { vehicle = pData.vehicle, parts = pSelectedParts } )
			end
		end)
...
addEvent("ShowUI_SpecialWorkshop", true)
addEventHandler("ShowUI_SpecialWorkshop", root, ShowUI_Workshop)
...
function ShowUI_OnFootWorkshop( state, data )
	if state then
		...
		for i, vehicle in pairs(pData.vehicles) do
			...
			local g_name = ibCreateLabel( 50, 0, 0, 90, pVehicleConf.model, g_item, 0xFFFFFFFF, 1, 1, "left", "center"):ibData("font", ibFonts.bold_18)
			local pby = 0
			for k, v in pairs(vehicle.operations) do
				local item = ibCreateImage( 0, pby, 500, 45, nil, g_item, 0x00000000 )
				local operation = ibCreateLabel( 260, 0, 0, 45, v.name, item, 0xFFffde9e, 1, 1, "left", "center" ):ibData("font", ibFonts.regular_16)
				local cost = ibCreateLabel( 440, 0, 0, 45, format_price( v.cost ) or 10000, item, 0x70FFFFFF, 1, 1, "left", "center" ):ibData("font", ibFonts.regular_16)
				local icon_money = ibCreateImage( 400, 12 , 27, 23, "files/img/icon_money.png", item )
		...
		:ibOnClick( function( button, state ) 
			if button == "left" and state == "down" then
				...
				triggerServerEvent( "OnPlayerTryRepairSpecialVehicle", localPlayer, { on_foot = true, operations = pSelectedOperations } )
			end
		end)
	else
		if isElement(ui.main) then
