...
function OnPlayerHitSpecialWorkshopMarker( workshop_id )
	...
	if not isElement(pVehicle) then
		local function GetVehiclesFromDatabase( query, pPlayer )
			...
			for k,v in pairs(data) do
				if special_allow[ IsSpecialVehicle( v.model ) ] then
			...
			triggerClientEvent( pPlayer, "ShowUI_SpecialWorkshop", pPlayer, true, { vehicles = pSpecialVehicles, on_foot = true } )
		end
	...
	triggerClientEvent( client, "ShowUI_SpecialWorkshop", client, true, pDataToSend )
end
addEvent("OnPlayerHitSpecialWorkshopMarker", true)
addEventHandler("OnPlayerHitSpecialWorkshopMarker", root, OnPlayerHitSpecialWorkshopMarker)

function OnPlayerTryRepairSpecialVehicle( data )
	...
	if data.on_foot then
		...
		if pPlayer:TakeMoney( iTotalCost, "aircraft_repair" ) then
			...
			triggerClientEvent( pPlayer, "ShowUI_SpecialWorkshop", pPlayer, false, { on_foot = true } )
		else
	...
	if pVehicle:IsInFaction() or pPlayer:TakeMoney( iCost, "aircraft_repair" ) then
		...
		triggerClientEvent( pPlayer, "ShowUI_SpecialWorkshop", pPlayer, false )
...
addEvent( "OnPlayerTryRepairSpecialVehicle", true )
addEventHandler( "OnPlayerTryRepairSpecialVehicle", root, OnPlayerTryRepairSpecialVehicle )
