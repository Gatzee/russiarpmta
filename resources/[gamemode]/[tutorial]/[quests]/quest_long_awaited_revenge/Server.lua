...
function RestoreWeapon( player, quest_weapons )
	...
	if data then
		...
		for k, v in ipairs( data.weapon_data ) do
			player:GiveWeapon( v.weapon_id, v.weapon_ammo, true, false, "weapon_restore_long_awaited_revenge" )
		end
...
function GiveQuestWeapon( player, weapon_data )
	...
	for k, v in ipairs( weapon_data ) do
		...
		player:GiveWeapon( v.weapon_id, v.ammo, true, true, "quest_long_awaited_revenge" )
	end
