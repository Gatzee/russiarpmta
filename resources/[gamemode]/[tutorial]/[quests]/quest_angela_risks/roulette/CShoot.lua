local pDefaultGunPosition = { -87.002, -470.934, 913.92, 90, 90, 0 }
...
function CreateGun()
	pGun = createObject( 1339, unpack(pDefaultGunPosition) )
	...
	elseif iAnimationStage == 3 then -- Выстрел
		...
		if fProgress >= 1 then
			...
			if pData.result then
				...
				playSound( ":nrp_casino_game_roulette/sfx/shot"..math.random(1,2)..".wav" )

				if pData.player == localPlayer then
					YouDied()
				end
			else
				playSound( ":nrp_casino_game_roulette/sfx/click"..math.random(1,3)..".wav" )
			end

			pLastGunPosition = { x, y, 914.1, 85, 0, 0 }
			pTargetGunPosition = { -87.002+math.random(-2, 2)/10, -470.934+math.random(-2, 2)/10, 913.92, 90, 360+math.random(0,180), 0 }
...
function YouDied()
	playSound( ":nrp_casino_game_roulette/sfx/youdied.wav" )
	pMemeFont = dxCreateFont(":nrp_casino_game_roulette/img/font.ttf", 69)
