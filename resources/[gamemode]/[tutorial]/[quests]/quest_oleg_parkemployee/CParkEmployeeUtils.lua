...
function CreateSurfacePaint( conf )
    ...
    self.sprite = self.sprite or ":task_park_employee_company_1/img/texture1.png"
    self.GRASS_SHADER = dxCreateShader( ":task_park_employee_company_1/fx/grass.fx", 0, 50, true, "world,object" )
    ...
    if not isElement( self.GRASS_SHADER ) or not isElement( self.GRASS_RT ) then
        triggerServerEvent( "PlayerFailStopQuest", localPlayer, { type = "fail_quest", fail_text = "Системная ошибка инициализации стрижки травы" } )
        return
    end
    ...
    self.func_paint = function()
        ...
        dxDrawImage( tx - self.size_sprite / 2, ty - self.size_sprite / 2, self.size_sprite, self.size_sprite, self.sprite , localPlayer.rotation.z, 0, 0, 0xFF00FF00 )
        ...
        if ticks > self.timeout  then
            ...
            for i = 0, self.pixels - 1 do
                for j = 0, self.pixels - 1 do
                    local r, g = dxGetPixelColor( pixels, i, j )
                    if r and g > 128 then 
            ...
            if procent >= 0.85 then
                ...
                if self.last_percent  ~= new_percent then 
                    localPlayer:ShowInfo( "Вы скосили ".. new_percent .."% от всего участка" )
