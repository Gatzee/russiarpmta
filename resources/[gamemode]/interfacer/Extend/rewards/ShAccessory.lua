...
REGISTERED_ITEMS.accessory = {
	...
	Give = function( player, params )
		player:AddOwnedAccessory( FixAccessoryID( params.id ) )
	end;

	IsValid = function( self, item )
		local params = item.params or item

		if not CONST_ACCESSORIES_INFO[ FixAccessoryID( params.id ) ] then
			return false, "Аксессуар с указанным ID не найден"
		end
	...
	checkHasItem = function( player, params )
		return player:GetOwnedAccessories( )[ FixAccessoryID( params.id ) ]
	end;
	...
	uiCreateItem = function( id, params, bg, sx, sy )
		...
		return ibCreateContentImage( 0, 0, csx, csy, id, FixAccessoryID( params.id ), bg ):center( )
	end;
