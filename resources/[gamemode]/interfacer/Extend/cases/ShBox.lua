...
REGISTERED_CASE_ITEMS.box = {
	rewardPlayer_func = function( player, params )
		for id, param in pairs( params.items ) do
			REGISTERED_CASE_ITEMS[ id ].rewardPlayer_func( player, param )
		end
	end;
	...
	uiCreateRewardItem_func = function( id, params, bg, fonts )
		...
		for id, param in pairs( params.items ) do
			...
			REGISTERED_CASE_ITEMS[ id ].uiCreateItem_func( id, param, min_bg, fonts )

			local description_data = REGISTERED_CASE_ITEMS[ id ].uiGetDescriptionData_func( id, param )
