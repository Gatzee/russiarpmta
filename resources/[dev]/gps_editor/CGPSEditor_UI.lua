...
function ShowGPSEditorUI( state )
    if state then
        ShowGPSEditorUI( false )

        UI_elements = {}
        UI_elements.black_bg = ibCreateBackground( 0xCC000000, ShowGPSEditorUI, true, true )
            ...
            :ibOnClick( function( key, state )
                if key == "right" then
                    ...
                    elseif state == "down" then
                        ...
                        --[[
                        local world_pos = GetWorldPosition()
                        GetPath( world_pos.x, world_pos.y )
                        --]]
                ...
                elseif key == "left" then
                    if getKeyState( "lctrl" ) then
                        ...
                        if state == "down" then
                            local world_pos = GetWorldPosition()
                            --[[
                            target_node = GetNodeByWorldPosition( world_pos.x, world_pos.y )
                            --]]
                            ...
                            if add_node then
                                if getKeyState( "lshift" ) then
                                    local is_add = AddNeighbours( UI_elements.last_node_id, add_node, getKeyState( "lalt" ) )
                                    if is_add then UI_elements.update_nodes() end
                                    UI_elements.last_node_id = add_node
                                else
                                    UI_elements.last_node_id = nil
                                end
                                UI_elements.update_nodes()
                            end
                            --]]
        ...
        UI_elements.update_nodes = function()
            dxSetRenderTarget( UI_elements.overlay_texture, true )
                ...
                --[[
                for i = 1, #DRAW_PATH do
                    local start_node = DRAW_PATH[ i ] 
                    local finish_node = DRAW_PATH[ i + 1 ]
...
addCommandHandler( "gps_editor", function()
    ShowGPSEditorUI( true )
end )