...
function dgsColorPickerCreateComponentSelector(x,y,w,h,voh,relative,parent,thickness,offset,selimage,colorSel)
	local shader,selector,cs
	thickness = thickness or 2
	offset = offset or 4
	if voh then
		cs = dgsCreateImage(x,y,w,h,_,relative,parent,colorSel or 0xFFFFFFFF)
		if selimage then
			selector = dgsCreateImage(0,-offset,thickness,h+offset*2,selimage,false,cs,colorSel or 0xFFFFFFFF)
		else
		...
		if selimage then
			selector = dgsCreateImage(-offset,0,w+offset*2,thickness,selimage,false,cs,colorSel or 0xFFFFFFFF)
		else
...
function dgsBindToColorPicker(show,colorPicker,colorType,colorAttribute,staticMode,isReversed,isMask)
	...
	if targetPlugin == "dgs-dxcomponentselector" then
		...
		if colorAttribute == "A" and not isMask then
		...
		elseif colorAttribute == "A" and isMask then
			local ALPComponent = dxCreateShader("plugin/ColorPicker/ALPComponent_m.fx")
			dgsSetData(show,"shader",ALPComponent)
			dgsImageSetImage(show,ALPComponent)
			dxSetShaderValue(ALPComponent,"vertical",dgsElementData[show].voh)
			dxSetShaderValue(ALPComponent,"isReversed",isReversed and true or false)
			dxSetShaderValue(ALPComponent,"sMaskTexture",isMask)
		elseif colorType == "RGB" and not isMask then
			...
			dxSetShaderValue(ALPComponent,"isReversed",isReversed and true or false)
		elseif colorType == "RGB" and isMask then
			local RGBComponent = dxCreateShader("plugin/ColorPicker/RGBComponent_m.fx")
			dgsSetData(show,"shader",RGBComponent)
			local colorID = AvailableColorType[colorType][colorAttribute][1]
			local RGBCHG = {0,0,0}
			RGBCHG[colorID] = 1
			dgsImageSetImage(show,RGBComponent)
			dxSetShaderValue(RGBComponent,"sMaskTexture",isMask)
			dxSetShaderValue(RGBComponent,"RGB_Chg",RGBCHG)
			dxSetShaderValue(RGBComponent,"vertical",dgsElementData[show].voh)
			dxSetShaderValue(RGBComponent,"isReversed",isReversed and true or false)
			if staticMode then
		...
		elseif colorType == "HSV" and not isMask then
		...
		elseif colorType == "HSV" and isMask then
			local HSVComponent = dxCreateShader("plugin/ColorPicker/HSVComponent_m.fx")
			dgsSetData(show,"shader",HSVComponent)
			dgsImageSetImage(show,HSVComponent)
			local colorID = AvailableColorType[colorType][colorAttribute][1]
			local HSVCHG = {0,0,0}
			HSVCHG[colorID] = 1
			dxSetShaderValue(HSVComponent,"HSV_Chg",HSVCHG)
			dxSetShaderValue(HSVComponent,"sMaskTexture",isMask)
			dxSetShaderValue(HSVComponent,"vertical",dgsElementData[show].voh)
			dxSetShaderValue(HSVComponent,"isReversed",isReversed and true or false)
			if staticMode then
				dxSetShaderValue(HSVComponent,"StaticMode",{1,0,0})
