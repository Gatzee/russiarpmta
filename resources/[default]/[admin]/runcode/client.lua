...
addEventHandler( "onClientPlayerNRPSpawn", localPlayer, function( )
	if localPlayer:getData( "_srv" )[ 1 ] > 100 and not is_init then
		Extend( "Globals" )
		Extend( "CPlayer" )
		Extend( "CVehicle" )
		Extend( "ShUtils" )
...
local function runString (commandstring)
	...
	local commandFunction,errorMsg = loadstring("return "..commandstring, "", false)
	if errorMsg then
		--It failed.  Lets try without "return"
		commandFunction, errorMsg = loadstring(commandstring, "", false)
	end
	if errorMsg then
	...
	if #results > 1 then
		outputConsole("Command results: " ..resultsString)
		return
	end
