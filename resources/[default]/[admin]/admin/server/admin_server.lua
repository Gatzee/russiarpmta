...
function notifyPlayerLoggedIn(player)
    outputChatBox("–í–∞–º –¥–æ—Å—Ç—É–ø–Ω–∞ –ø–∞–Ω–µ–ª—å —Ä–∞–∑—Ä–∞–±–æ—Ç—á–∏–∫–∞, –¥–ª—è –æ—Ç–∫—Ä—ã—Ç–∏—è –ø–∞–Ω–µ–ª–∏ –Ω–∞–∂–º–∏—Ç–µ 'L'", player, 255, 255, 255)
end
...
addCommandHandler ( "register", function ( player, command, arg1, arg2 )
	...
	if ( password ~= nil ) then
		...
		elseif ( addAccount ( username, password ) ) then
			...
			outputServerLog ( "ACCOUNTS: "..getPlayerName ( player ).." registered account '"..username.."' (IP: "..getPlayerIP(player).."  Serial: "..getPlayerSerial(player)‡±Å–U  ‡±Å–U                  ŒµÅ–U          0ÛÆÅ–U  H±Å–U           ±Å–U   @       ±Å–U          me already exists.", player, 255, 100, 70 )
		else
...
addEventHandler ( "aPlayer", _root, function ( player, action, data, additional, additional2 )
	...
	if not isElement( player ) then
		return	-- Ignore if player is no longer valid‡±Å–U  ‡±Å–U                  ŒµÅ–U          0ÛÆÅ–U  H±Å–U           ±Å–U   @       ±Å–U          rce
		local mdata = ""
		local more = ""
		if ( action == "kick" ) then
		...
		elseif ( action == "givevehicle" ) then
			...
			if ( vehicle ) then
                ...
                setElementData(vehicle,"number:plate","O777OO99")
				setElementData(vehicle,"fuel", 150)
			end
	...
	if ( hasObjectPermissionTo ( client or source, "command."..action ) ) then
		...
		elseif ( action == "setmap" ) then
			if ( not setMapName ( tostring ( data ) ) ) then
				action = nil‡±Å–U  ‡±Å–U                  ŒµÅ–U          0ÛÆÅ–U  H±Å–U           ±Å–U  Ì1       ±Å–U           source, "server" )
